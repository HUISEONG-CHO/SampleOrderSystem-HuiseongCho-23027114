...
```
[RED]  테스트 작성 → 테스트 실행(FAIL 확인) → 사용자 보고 & 승인 → 커밋
                                                        ↓
[GREEN]               구현 작성 → 테스트 실행(PASS 확인) → 사용자 보고 & 승인 → 커밋
```

> RED에서 테스트가 FAIL하지 않으면 잘못 작성된 테스트다. 반드시 실패를 확인한 뒤 커밋한다.  
> **테스트 실행 후에는 반드시 결과를 사용자에게 보고하고 승인을 받은 뒤 다음 단계로 진행한다.**

#### 테스트 결과 저장 규칙

모든 테스트 실행 결과는 해당 PoC 서브디렉터리의 `test_result/` 폴더에 txt 파일로 저장한다.

- 파일 경로: `{PoC 서브디렉터리}/test_result/{Phase명}_{RED|GREEN}.txt`
- FAIL 시 원인을 반드시 명시한다.
...
3. **결과를 `test_result/{Phase명}_RED.txt`에 저장**
4. **사용자에게 FAIL 결과 보고 후 승인 대기**
5. 승인 후 커밋
...
2. 테스트 실행 → **PASS 확인 필수**
3. **결과를 `test_result/{Phase명}_GREEN.txt`에 저장**
4. **사용자에게 PASS 결과 보고 후 승인 대기**
5. 승인 후 커밋
